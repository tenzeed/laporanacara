...
-- ---------------------------------------------------------
-- Panel Admin — PIN master milik SATU pengelola utama aplikasi
--
-- Terpisah total dari PIN per acara. Admin bisa mereset PIN acara
-- mana pun (tanpa perlu tahu PIN lama acara itu) atau menghapus acara
-- mana pun, khusus untuk pemulihan/pembersihan saat bendahara lupa
-- PIN atau lupa menghapus acara yang sudah tidak dibutuhkan.
--
-- pin_hash admin TIDAK diisi otomatis di sini. Setelah menjalankan
-- migrasi/skema ini, jalankan SEKALI secara terpisah (ganti teks
-- contoh dengan PIN rahasiamu sendiri, minimal 8 karakter):
--
--   insert into app_admin (id, pin_hash)
--   values (1, crypt('GANTI-DENGAN-PIN-RAHASIA-KAMU', gen_salt('bf')))
--   on conflict (id) do nothing;
--
-- Simpan PIN itu baik-baik — ini kunci induk seluruh aplikasi.
-- ---------------------------------------------------------
...
-- Sengaja tidak ada policy sama sekali -> tidak ada akses langsung dari
-- klien; hanya fungsi security definer di bawah yang bisa membacanya.
...
begin
  ...
  if v_hash is null then
    return false; -- admin PIN belum pernah di-setup
  end if;

  if v_locked_until is not null and v_locked_until > now() then
    return false; -- masih dikunci sementara karena terlalu banyak percobaan gagal
  end if;
